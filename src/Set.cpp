...
void Set::preflow(std::shared_ptr<Node> origin)
{
    ...
    for(auto it = m_adj[origin_index].begin(); it != m_adj[origin_index].end(); ++it) {
        ...
        edge->node->addExcessFlow(edge->flow);

        addEdge(edge->node->getId(), origin->getId(), -edge->flow, 0);
    }
}
...
bool Set::push(std::shared_ptr<Node> node)
{
    ...
    for(auto it = m_adj[origin_index].begin(); it != m_adj[origin_index].end(); ++it) {
        ...
        if(node->getHeight() > destination_node->getHeight()) {
            int flow = MIN(edge->capacity - edge->flow, node->getExcessFlow());
            node->addExcessFlow(-flow);
            destination_node->addExcessFlow(flow);

            edge->flow += flow;

            updateReverseEdgeFlow(node->getId(), destination_node->getId(), flow);
...
void Set::relabel(std::shared_ptr<Node> node)
{
    ...
    for(auto it = m_adj[origin_index].begin(); it != m_adj[origin_index].end(); ++it) {
        ...
        auto destination_node = edge->node;
        if(destination_node->getHeight() < minimum_height) {
            minimum_height = destination_node->getHeight();
...
int Set::getMaxFlow()
{
    ...
    for (int i = 0; i < m_num_nodes; i++) {
        for(auto it = m_adj[i].begin(); it != m_adj[i].end(); ++it) {
            auto edge = (*it);
            int origin_id = i;
            int destination_id = edge->node->getId();
